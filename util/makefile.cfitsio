...
ifeq ($(origin CFITS_INC), environment)
  ...
  X := $(shell $(PKG_CONFIG) --exists cfitsio && echo yes || echo no)
  ifeq ($(X), yes)
    CFITS_EXISTS ?= 1
  endif
endif

CFITS_INC ?= $(shell $(PKG_CONFIG) --cflags cfitsio 2>/dev/null)
CFITS_LIB ?= $(shell $(PKG_CONFIG) --libs cfitsio 2>/dev/null)
CFITS_SLIB :=
